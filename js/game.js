...
function getPointGen() {
	...
	if(hasUpg("p",21)) gain = gain.times(upgEffect("p",15))
	return gain
}
...
function getNextAt(layer, canMax=false, useType = null) {
	...
	if (type=="static") 
	{
		...
		next = next.root(tmp[layer].gainMult.div(tmp[layer].gainExp)).root(layers[layer].exponent).times(tmp[layer].requires).max(tmp[layer].requires)
